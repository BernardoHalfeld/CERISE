 ...
 *
 * Estratégia: Cada módulo varre de forma independente o espectro completo de 0 a 78,
 * criando um sistema redundante e uma interferência de banda larga.
 * Se um módulo falhar, os outros continuarão a fornecer cobertura total.
 *
 * Rádio 1 (Pinos 4, 5)   -> LED 1 (Pino 13)
 * Rádio 2 (Pinos 26, 27) -> LED 2 (Pino 12)
 * Rádio 3 (Pinos 32, 33) -> LED 3 (Pino 14)
 *
 */
...
// Cada rádio terá seu próprio contador para varrer o espectro completo
byte canalAtual1 = 0;
byte canalAtual2 = 26; // Começa em um ponto diferente para dessincronizar
byte canalAtual3 = 52; // Começa em um ponto diferente para dessincronizar
...
void setup() {
  Serial.begin(115200);
  while (!Serial);
  Serial.println("--- Scanner de Espectro Redundante (0-78) com 3x NRF24L01 ---");
...
void loop() {
  // --- VARREDURA DO MÓDULO 1 (Canais 0-78) ---
  canalAtual1++;
  if (canalAtual1 > 78) {
    canalAtual1 = 0;
  }
  ...
  // --- VARREDURA DO MÓDULO 2 (Canais 0-78) ---
  canalAtual2++;
  if (canalAtual2 > 78) {
    canalAtual2 = 0;
  }
  ...
  // --- VARREDURA DO MÓDULO 3 (Canais 0-78) ---
  canalAtual3++;
  if (canalAtual3 > 78) {
    canalAtual3 = 0;
  }
